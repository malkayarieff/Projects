...
DS-399 Major Capstone Project Student: Malka Yehudis Arieff Instructor: Sharath Kumar Jagannathan July 2026
...
NLP keyword-based risk scoring system (STABLE / MONITOR / ELEVATED / HIGH RISK)
Logistic regression model identifying the drivers of HIGH RISK classifications (F1 = 0.903, AUC-ROC = 0.985)
Interactive Streamlit dashboard for per-patient risk lookup and monitoring
Files
File	Description
...
Written_Report_final.pdf	Full written report
Presentation_Slides_final.pdf	Final presentation slides
README.md	This file

Note on data: The underlying EHR data (nursing notes, diagnosis and medication records, and model output tables) is not included in this repo, since it contains real patient information from a long-term care facility. See the Data Dictionary above for field-level documentation of what the data contained.

How to Run
1. Run the Notebook (Google Colab)
Open Google Colab
Upload Final_Code_EDA_Model.ipynb
Upload the source data files (diagnosis.csv, medication.csv, notes.csv) — not included here; see note above
Run all cells in order (Runtime → Run all)
The notebook will generate patient_risk_scores_full.csv and all chart files
2. Run the Dashboard (Local)
Install dependencies:
bash
pip install streamlit pandas plotly
Put dashboard_app.py and a generated patient_risk_scores_full.csv in the same folder
Run:
bash
cd /path/to/folder
